            ...
            >
                ...
                <main class="p-6 space-y-6">
                    ...
                    <div class="bg-white rounded-xl border shadow-sm max-w-2xl">
                        <form method="POST"
                              action="{{ route('admin.template-surat.store') }}"
                              class="p-6 space-y-6">
                            @csrf
                            ...
                            <div>
                                ...
                                <input
                                    type="text"
                                    name="kode_template"
                                    value="{{ old('kode_template') }}"
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2
                                           focus:border-indigo-500 focus:ring-indigo-500"
                                    placeholder="Contoh: TMP-UND-001"
                                    required
                                >
                                @error('kode_template')
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    class="w-full rounded-lg border border-slate-300 px-3 py-2
                                           focus:border-indigo-500 focus:ring-indigo-500"
                                    placeholder="Surat Permohonan Pengadaan"
                                    required
                                >
                                @error('nama_template')
                                    <p class="text-sm text-red-600 mt-1">{{ $message }}</p>
                                ...
                                >
                                    ...
                                    @foreach($alurVerifikasi as $alur)
                                        <option
                                            ...
                                            {{ old('kode_alur') == $alur->kode_alur ? 'selected' : '' }}
                                        >
                                            {{ $alur->nama_alur }}
                                        </option>
                                    @endforeach
